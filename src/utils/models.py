...
from .layers import DivisiveNormalization2d, AdaptiveThreshold, Normalize
...
class Matching_VGG(nn.Module):
            ...
            if isinstance(layer_info, str):
                ...
                if type(self.divisive_sigma)==float and layer_i <= 7:
                    ...
                    if self.threshold is not None and self.threshold>0.0:
                        threshold_layer = [ AdaptiveThreshold(mean_scalar=self.threshold)]
                else:
    ...
    def name(self) -> str:
        if self.conv_layer_name != "conv2d":
            s = f"{self.conv_layer_name}_"
        else:
            s = ""
        if type(self.divisive_sigma)==float:
            s += f"divisive_{self.divisive_sigma}"
            if self.threshold is not None and self.threshold>0.0:
